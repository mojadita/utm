 ...
 * * Corregida errata en el c√°lculo de N por FFT.
 * * Corregido pru.c para que muestre el tama√±o de la estructura struct
 *   utmparam.
 *
 ...
 * * Introduciendo la sustituci√≥n del c√°lculo de las series de Fourier por
 *   el c√°lculo de los coeficientes a partir de la FFT.
 *
 ...
 * * Incluidas las funciones para el c√°lculo de la FTT del m√≥dulo fft.
 *   A√∫n no est√°n integradas.
 *
 ...
 * AÒadidos utm_ini.h para que genutm pueda calcular por tabla los par·metros y
 ...
 * RevisiÛn con c·lculo de coordenadas UTM a partir de geodÈsicas y
 * determinaciÛn del recuadro cienkilomÈtrico y la zona.
 *
 * Revision 2.0  1998/05/12 18:53:09  luis
 * VersiÛn directa completa con c·lculo de coordenadas UTM a partir
 * de geodÈsicas.
 *
 * Revision 1.2  1998/05/11 18:45:00  luis
 * C·lculo directo completo.
 *
 */
...
/********* TRANSFORMACI”N GEOD…SICAS A UTM *****************/
...
/************** TRANSFORMACI”N UTM -> GEOD…SICAS ****************/

/* FUNCI”N ATEB, INVERSA DE LA FUNCI”N BETA */
...
int main(int argc, char **argv)
{
  ...
  while((opt = getopt(argc, argv, "g:")) != EOF) {
    ...
    case 'g': sg=lookup(optarg);
		if(!sg) {
			fprintf(stderr, "Sistema GeodÈsico desconocido(%s)\n", optarg);
			exit(1);
		}
		break;
    default:
